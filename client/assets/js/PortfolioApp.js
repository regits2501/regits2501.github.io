...
angular.module('Portfolio.Common').factory('CleanLeftoverShimsService', function ($window, ORIENTATION_ON_LOAD, GetOrientationService) {
  ...
  function CleanLeftoverShims() {
    ...
    if (!browserIsSniffed) {
      if (ORIENTATION_ON_LOAD.value === currentOrientation)
        // Sniff for other browsers
        otherBrowser = true; // we are in some other browser (Chrome, FF...)

      if (ORIENTATION_ON_LOAD.value !== currentOrientation)
        // Sniff for certain safari versions
    ...
    if (otherBrowser) {
      ...
      if (currentOrientation === 'portrait')
        // We are actually in landscape mode, since orient.change
...
angular.module('Portfolio.Common').factory('EqualDimensionsService', function (CURRENT_SIDE, ADDRESS_BAR_HIDDEN, RESIZE_EVENT, NotShownPagesCleanService,
// set element's height and width 
SideWithOtherOverflowService,
  ...
  function EqualDimensions(args) {
    ...
    if (CURRENT_SIDE.value.name === pageName) {
      ...
      CURRENT_SIDE.toBeShown[pageName] = setTimeout(function removeEqualDimensions() {
        ...
        var overflowSide = SideWithOtherOverflowService(pageName);
